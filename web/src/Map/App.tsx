...
const App = (props: any) => {
  ...
  const handleCoordinateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isValidCoordinate(newLat) && isValidCoordinate(newLng)) {
      ...
      setCoordinate(updatedCoordinates); // Update coordinate state
      props.setCoordinate(newLat, newLng); // Sync with parent state
    }
  };
    ...
    <>
      ...
      <div className="mt-10" style={{ position: 'relative' }}>
        ...
        <input
          ...
          )} mb-4 w-full rounded-lg border-0 py-1.5 pl-4 shadow-md outline-none sm:max-w-full sm:text-sm sm:leading-6 lg:max-w-90`}
        />
        {attemptedSubmit && !isCoordinateValid(inputValue) && (
